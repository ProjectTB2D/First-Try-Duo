...
int main()
{
  ...
  int prad = 10;
  sf::CircleShape cs(prad);
  int pangle = 0;
  cs.setFillColor(sf::Color::Yellow);
  cs.setPosition(50, 50);
    ...
    while (app.isOpen())
    {
        ...
        while (app.pollEvent(event))
        {
			...
			if(event.type == sf::Event::KeyPressed && event.key.code == Keyboard::Up){
			  ...
			  cs.setPosition(cs.getPosition().x + 10*cos(pangle*3.14/180), cs.getPosition().y + 10*sin(pangle*3.14/180));

			  if(obs.collisionCircle(cs.getPosition(), prad))
